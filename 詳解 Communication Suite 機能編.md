...
- 音声の再生ボタンを押しても再生されない。
	...
	3 | PCの再生デバイスに問題がある。 | 再生中となっているのに音声が再生されない場合、そのPCに再生できるデバイスがない場合があります。**特にコンバージャ版利用している環境ではコンバージャが再生デバイスに指定されている可能性があります。** 再生可能なデバイスを接続して再生デバイスの設定をご確認ください。  
...
- 音声のマスクを有効にして、通話マスクを実行しても音声がマスクされない。
	...
	No. | 原因 | 対処 |
	----:|---------|-------|
	1 | 「再生に使用する音声ファイルのベースディレクトリのパス」のフォルダのACLに仮想ユーザ「SpeechVisualizer」 がアクセスできる権限がない。|該当フォルダのACLに対して「IIS AppPool\SpeechVisualizer」で追加可能な仮想ユーザ「SpeechVisualizer」を追加する 。  
