...
提供されたライセンスが使用されるタイミングは以下です。
	...
	3 | AmiVoice RealTimeRecorder |「内線番号管理」に内線番号を追加する毎に1つ使用。内線番号の登録数分使用。  |  
...
- CommunicationSuite ライセンスの登録  
	...
	![](images/Tips.jpg){width=50px}　下記ライセンスはライセンスファイルでアクティベーションされず、ライセンス状況には表示されませんが  
	機能を利用する場合には購入が必要です。  
...
<br>
  - 感情解析可能な通話数  
	購入ライセンス数 × 3000（バッチ版は以下参照） が感情解析可能な通話数としてシステム反映されます。  
	リアルタイム版・・・購入したライセンス数 × 3000 回  
	バッチ版・・・購入した **最初の1ライセンスのみ 30,000 回** 。以降1ライセンス追加ごとに 3,000 ずつ追加
。  
	![](images/Check.png){width=50px}　感情解析ライセンスは月単位の利用形態となり、これは月ごとの感情解析利用回数です。ライセンスの利用状況は （[@fig:kanjo]） の今月の感情解析通話数より確認できます。消費した感情解析通話数は毎月1日の午前9時にリセットされて「0」に戻ります。利用可能なライセンス数を超過した場合には翌月のライセンスリセットまで、それ以降の通話の感情解析が実行されません。  
<br>
  - 感情解析通話数の消費タイミング  
	基本的には **StreamingRecgnizer の認識処理実行時** に消費されます。(例外は後述 Tip 参照)

	録音構成（通話プロバイダ）や通話パターンによって消費数が変わります。  
<br>
	1. 保留操作時の通話パターン  
   		- 保留イベントを取得できるパターン  
	![](images/3-7-kanjoz1.png){#fig:kanjoz1 width=650px}  
...
![](images/3-7-kanjoz3.png){#fig:kanjoz3 width=650px}  
	...
	1. 転送時の通話パターン  
	転送時の通話パターンも保留イベントを取得できるかで消費数が変わります。  
