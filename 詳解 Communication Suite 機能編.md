...
以下の2箇所に表示されます。  
	...
	![](images/Check.png){width=50px}　『通話詳細の折れ線グラフに表示する感情（オペレータ）』 と 『通話詳細の折れ線グラフに表示する感情（カスタマ）』 の表示に使用される感情は 『|感情解析結果ファイルのベースディレクトリのパス』 で指定したフォルダに格納される、拡張子 『emo』 のファイルに保存されているデータが使用されます。<br />emo 形式ファイルは通話ごとに作成され、表示可能な全感情の時系列データで構成されており、[@tbl:emoscore] の 『保存する感情スコア』 の設定にない感情であっても指定・表示が可能です。
...
1. 通話内容ビューのトラブルシューティング
	...
	- 認識テキストが表示されない。
		...
		No. | 原因| 切り分けと対処 |
		----:|---------|-------|
		1 | 音声認識エンジンが設定もしくは適用されていない。  | ControlCenterの「認識オプションの設定 - システムデフォルト」を確認してください。「テキスト認識」が「無効」もしくは「音声認識エンジンモード」が「未設定」の場合、テキスト認識が行われません。認識エンジンを設定後、「ノード管理」から設定の再読み込みを実施したのち、通話の再認識を実施してください。   |
		2 | DBの容量が不足している。  | ControlCenterの「ノード管理」を確認して、「認識完了」に通話が残存している場合に該当します。SQLServerManegementStdioを使用してDBの使用量を確認してください。定義された領域をすべて使用している可能性があります。領域の拡張と必要であればHDDの拡張をお願いいたします。  |
		3 | StreamingRecognizerからのアップロードに失敗している。  |  ControlCenterの「ノード管理」を確認して、「認識エラー」に通話が残存している場合に該当します。  サーバあるいはネットワーク障害等の要員により、認識テキストのアップロードに失敗しています。TaskRunnerの「RecognitionQueueRecovery」を実行してください。|

		: 認識テキスト表示 のトラブルシューティング {#tbl:recognitiontexttrouble}

	<br />  

	- 音声の再生ボタンが非活性になっている。( ボタンがクリックできない。 )

		No. | 原因| 切り分けと対処 |
		----:|---------|-------|
		1 | ログインユーザに当該通話を再生する権限がない  | メッセージがなく、ボタン非活性の場合が該当します。意図していない動作であればControlCenterの「ユーザ管理」「ロール管理」を確認して「通話の再生」権限をユーザに付与する必要があります。  |
		2 | Internet Explorer 11 以外のブラウザを利用している。 |「Windows Media Player が起動できないため再生できません。」というメッセージが表示されている場合、音声再生の動作対象外ブラウザを利用している可能性があります。音声再生を利用する場合はInternet Explorer 11を利用してください 。  
		3 | WindowsMediaPlayer がインストールされていない | IE11 利用中かつ「Windows Media Player が起動できないため再生できません。」というメッセージが表示されている場合に該当します。再生には WindowMediaPlayer が使用されます。WindowsMediaPlayerのインストール状況を確認してください。  
		4 | IE11 の WindowMediaPlayer アドオンが有効になっていない | No.3の状況でWindowsMediaPlayerがインストール済でも再生できない場合に該当します。WindowsMediaPlayerがインストールされていてもアドオンが有効化されていない場合は実行できません。IE11 の 「アドオンの管理」 から有効化を実施してください。  

		: 音声再生 ( ボタン非活性 ) のトラブルシューティング {#tbl:audioplaybottuntrouble}
	...
	- 音声の再生ボタンを押しても再生されない。
		...
		No. | 原因| 切り分けと対処 |
		----:|---------|-------|
		1 | 再生用音声ファイルが存在しない。  | ボタンクリック後、「プレイヤーでファイルがみつからないため再生できません。」のメッセージが表示される場合に該当します。再生先の音声ファイルが保存期間を超えて削除されている等の理由で「再生に使用する音声ファイルのベースディレクトリのパス」のフォルダに音声ファイルが存在しない場合、再生できません。 基本的には同タイミングで波形も表示されなくなります。 |
		2 | 再生用音声ファイルにアクセスできない。 |「Windows Media Player が起動できないため再生できません。」というメッセージが表示されているかつ再生用音声ファイルが存在する場合、「再生に使用する音声ファイルのベースディレクトリのパス」のフォルダへのアクセス制限、NW障害等の理由で再生できない場合があります。当該フォルダのセキュリティやNW状況を確認してみてください。  
		3 | PCの再生デバイスに問題がある。 | 画面上では再生中となっていて、再生バーが移動しているのに音声が再生されない場合、そのPCに再生できるデバイスがない、もしくは再生デバイスが正しく設定されていない場合があります。**特にコンバージャ版利用している環境ではコンバージャが再生デバイスに指定されている可能性があります。** 再生可能なデバイスを接続して再生デバイスの設定をご確認ください。  

		: 音声再生 ( ボタン活性 ) のトラブルシューティング {#tbl:audioplaytrouble}

		![](images/Check.png){width=50px}　セキュアな通信下では、通話詳細画面へ遷移後、初回の音声再生は**ダウンロードが完了した後に通話の冒頭から再生されます**。途中再生を選択していても必ず冒頭から再生されます。

	<br />  

	- 音声のマスクを有効にして、通話マスクを実行しても音声がマスクされない。

		No. | 原因 | 対処 |
		----:|---------|-------|
		1 | 「再生に使用する音声ファイルのベースディレクトリのパス」のフォルダの ACL に仮想ユーザ 「SpeechVisualizer」 がアクセスできる権限がない。|該当フォルダのACLに対して「IIS AppPool\SpeechVisualizer」で追加可能な仮想ユーザ 「SpeechVisualizer」 追加してください 。  

		: 通話マスクのトラブルシューティング {#tbl:waveviewtrouble}
...
内容は以下の通りです。  
	...
	- 最新版  
		...
		最新版でのみ認識テキストの修正と SpeechVisualizer 上での通話マスク機能が利用可能です。  
...
1. メッセージタブに表示されるメッセージについて
	...
	表示メッセージは通話に紐づけられて保存されており、通話データが削除されると、メッセージも削除されます。
