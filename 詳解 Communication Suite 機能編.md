...
[@tbl:ｓｖhomepara] の『ホームモジュールのプロバイダ』 の設定書式は以下です。
	...
	~~『ホームモジュールのプロバイダ』は、ホーム画面に設定可能にしたいガジェットのプロバイダ名ごとに、1つづつ改行して記載します。~~
	【要構成検討】 ここで設定したプロバイダのみがレイアウト対象になるという注意点は、レイアウトの方で説明した方が通りがいい。

	<br />

	~~**[@tbl:homeprovider]の内容以外に、重要な通話ガジェット(HighCallPriorityConversation)が用意されております。このガジェットは将来予定されているバージョンアップで追加予定の機能になります。現段階では表示のみのため、設定しないことを推奨いたします。**~~

	<br />

	また、一部のガジェットは内容の自動再読込の設定が可能です。[@tbl:svhomeautoupdate]は自動再読込間隔を設定する詳細設定項目です。


	No. | 設定項目名       | 設定値 | 内容 |
	---:|------------------|--------------|------|
	1   | ホームモジュールの自動再読込間隔  | 600000  | 各ホームモジュールが自動で再読込を行う場合の間隔を指定します。単位は「ミリ秒」  |

	: ホーム画面の自動更新設定 {#tbl:svhomeautoupdate}


#### 2-2-2. ホーム画面のレイアウト情報

- ホーム画面のガジェット配置を詳細設定から設定可能です。配置情報で使われるガジェット名は[@tbl:homeprovider]で記載したプロバイダ名と一部内容が異なります。[@tbl:layoutgadget]はプロバイダ名に対応するガシェット名の一覧です。
	...
	システム全体のレイアウト設定は詳細設定内で設定できます。表示したい列を`;`で区切り、列内のガジェット名を`,`で区切ります。以下はデフォルトの設定内容です。
	...
	![](images/Tips.jpg){width=50px}　ホーム画面のレイアウトはシステム全体のデフォルト設定とユーザごとの個人設定が有り、ユーザごとの個人設定が設定されている場合は、個人設定の内容が優先されます。

	<br />

 	1. プロフィール（Profile）

      	- ログインしているユーザのプロフィール設定です。ガジェット名はProfileになります。  
      	ユーザ個人設定の『ホーム画面のレイアウト』には以下のように表示されます。
 	...
 	1. リンク  
	      - ControlCenterで登録したリンクの設定です 。ログインしているユーザの所属プロジェクトによって表示する項目がコントロール可能です。ガジェット名はLinkになります。  
	[@fig:home_link] はリンクガジェット、[@tbl:svhomelink] はレイアウト配置設定項目です。リンクガジェットはガジェット名以外の設定項目を持ちません。
 	...
 	1. お知らせ  
	      ...
	      - ControlCenterで登録したお知らせの設定です 。ログインしているユーザの所属プロジェクトによって表示する項目がコントロール可能です。ガジェット名はAnnouncementになります。
 	...
 	1. 自分の通話  
	      - ログインしているユーザの通話一覧です 。ガジェット名はMyRecentになります。
 	...
 	1. 自分の通話(IN)  
      	...
      	- ログインしているユーザの着信した通話一覧です 。ガジェット名はMyRecentInboundになります。
[@fig:home_myrecentinbound] は自分の通話(IN)ガジェット、[@tbl:svhomemyrecentinbound] はレイアウト配置設定項目です。
 	...
 	1. 自分の通話(OUT)  
      	...
      	- ログインしているユーザの発信した通話一覧です 。ガジェット名はMyRecentOutboundになります。
	[@fig:home_myrecentoutbound] は自分の通話(IN)ガジェット、[@tbl:svhomemyrecentoutbound] はレイアウト配置設定項目です。
...
検索条件の機能です。
 ...
 1.  検索条件のオプション  
		検索条件を指定する際にオプションを使うことで、より高度な検索条件を指定できます。
