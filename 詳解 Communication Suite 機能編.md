...
話せば話すほど、話者の音響特性にカスタマイズされ、認識精度の向上が見込めます。

	![](images/NOTICE.png){width=50px}  

	1. GMM エンジンでのみ利用できます。それ以外のエンジンで利用するとアダプテーションは行われません。  
	1. ユーザデータに紐付いて保存されます。OperatorAgent を利用していないときには学習されません。  
	1. OperatorAgent にログイン中のユーザと別のユーザで通話を実施してしまうと、別の特徴をアダプテーションしてしまうため、認識精度の低下に繋がります。  
