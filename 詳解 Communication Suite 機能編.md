...
通話検索画面の UI で検索条件として指定する検索クエリの仕様について説明していきます。  
   ...
   2. 検索条件のオプション  
	       	[@tbl:option1] は検索時に利用可能なオプションです。  
				 各検索クエリごとに利用可能なオプションやオプションの仕様が異なります。  

		 No. | オプション     |説明
		 ---:|-----------------|----------|
		 1   |カンマ|  １つの検索条件内で複数条件を指定する
		 2   |スペース| 検索クエリを複数組み合わせて指定する  
 		 3   |ハイフン | ・1つの検索条件内で指定したものを除外する<br>・日付や時間で使用する場合は範囲指定となります
...
- ハイフン  
 ...
 3. フリーワード検索  
	認識結果に含まれる任意の単語を検索します。  
	フリーワード検索は検索クエリが不要となり、検索したい単語を検索条件に入力します。
	...
	No. | 論理演算子     |オプション |説明
	---:|-----------------|----------|--------|
	1   |論理積 ( AND 検索)|スペース|指定した2つ以上のフリーワードを条件とする   
	2   |論理和 ( OR 検索)|or|指定した2つ以上のフリーワードのいずれかを条件とする  
	3   |論理否定 （ NOT 検索） |ハイフン| 指定したフリーワードを除外する<br>

	: フリーワード検索のオプション {#tbl:option2}

	![](images/Tips.jpg){width=50px}　NOT検索のみの指定ではすべての単語が除外対象となるため、検索対象とする単語と組み合わせて利用する必要があります。
...
フリーワードの対象となる文字列は SQL Server 機能のワードブレーカーによって分割された単位が対象となります。  
	...
	![](images/Tips.jpg){width=50px}　ノイズワードが含まれていた場合にはフリーワード検索が正常に行われません。  ノイズワードとは、自然言語を処理するにあたって一般的であるなどの理由で処理対象外とする単語です。 日本語の 「は」 「の」 「です」 「その」 、英語の 「a」 「the」 「for」 「of」などです。  
