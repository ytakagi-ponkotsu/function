...
2 | 通話フィルタの起動条件を満たしていない |フィルタの条件で、起動条件を指定しますが、キーワードがテキスト化された時に指定した条件を満たしているか確認してください。   
...
7. 感情解析   
送話と受話側の発話内容をリアルタイムに感情で数値化して表示する機能です。  
感情解析は音声データから解析しており、テキスト化された文字を解析対象としていません。

- OperatorAgentで表示される機能 ([@fig:emo1])([@fig:emo２])([@fig:emo3])  
...
- 感情解析ライセンスの消費について  
  ...
  感情解析が利用できる通話数には上限があり、感情解析ライセンス数に依存します。  
  原則として Communication Suite が1通話とカウントした数だけ感情解析ライセンスが消費されます。  
...
　　「今月の感情解析通話数」は以下の計算式で算出されています。【要確認】  
　　例） 5 内線分のライセンス環境の場合  
　　１ヵ月あたり、5内線 × 3,000通話 = 15,000通話となります。  
　　※サーバ版の場合は、どの内線での録音かは関係なく、合計の通話数が 15,000になるまで処理されます。  
　　クライアント版の場合は、内線数に関係なく、クライアント版のラインセンス数(=インストール数)となります。



　　１つの通話でも、以下のような条件下でさらに感情解析ライセンスが消費されます。  

　　・ 保留により通話が分割された場合（保留前、保留後で２つライセンスを使用）【要確認】  
　　・ 通話を転送した場合（転送元、転送先で２つライセンスを使用）  
　　・ 再認識を実施した通話（ConrtolCenter の認識オプションの設定で感情解析が有効の場合）  
　　・ 電話機のモニタ機能で録音されたモニタリング側の通話  
　　・ 「短い通話の録音キャンセル」機能によりキャンセルされた通話（CCの設定【要確認】）  
　　・ 「長い通話の分割」機能により分割された通話（CCの設定【要確認】）   


- 感情解析の対象範囲  
  １通話内で感情解析の対象とする範囲を設定することができます。  
  設定はControlCenter / 認識オプション にあります。（[@fig:ninsiki1])（[@fig:ninsiki2]）   


	![認識オプション（オペレータ） 画面](images/2-1-認識オプション1.png){#fig:ninsiki1 width=400px}  


	![認識オプション（カスタマ） 画面](images/2-1-認識オプション2.png){#fig:ninsiki2 width=400px}  


[@tbl:emosco] は、保存する感情スコアの設定項目です。ControlCenter の詳細設定項目にあります。  

  No. | 設定分類| 設定項目名                | 設定内容      |
  ----:|---------------------|------------------|--------------|
  1   |共通 - 感情解析| 保存する感情スコア| （感情解析識別名）.（話者)で指定    

: 保存する感情スコア {#tbl:emosco}  
...
#### 2-1-1. SpeechVisualizer のログインに関連する ControlCenter の詳細設定項目
  - [@tbl:svlogin] は、SpeechVisualizer のログインに関わる詳細設定項目です。設定分類は、**共通 - セキュリティ** となります。
	...
	![](images/NOTICE.png){width=50px}　[@tbl:svlogin] の No1 を true に設定すると、ログイン画面に 『ブラウザを終了してもログインを維持する』 というチェックボックスが表示されます。その項目にチェックをつけてからログインするとこで、ユーザの Cookie が永続化されます。  
...
- [@tbl:svlogin] は、SpeechVisualizer のログインに関わる詳細設定項目です。設定分類は、
	...
	No. | 設定項目名       | デフォルト値 | 内容 |
	---:|------------------|--------------|------|
	1   |  ログイン状態の維持 | 1800        | 【要確認】 |
	2   |  ブラウザログインを継続するための通知間隔 | 20        | 【要確認】 |
	3   |  ログイン維持の通知間隔 | 180       | 【要確認】 |
	4   |  ログイン状況タイムアウト | false        | ログインダイアログに内線番号入力欄を表示する |
	5   |  ログイン状態の維持 | S        | R = 必須, S = サーバ版では必須, N = 入力しない |
