...
狭義としてのバッチ認識は、MediaScriber での処理や復旧認識のように認識処理開始時点で既にテキスト化対象の通話が終了しており、認識音源に静的な音声ファイルを利用する処理です。この場合、認識要求種別が **バッチ** になります。  

		![認識要求種別の種類](images/3-4_recognitionrequest_batch.png){#fig:ecognitionrequest_batch width=400px}
...
- Communication Suite ライセンス  
	Communication Suite ライセンスは ControlCenter / システム管理 / ライセンス状況のホーム画面より確認できます。（[@fig:license01]）  

	![ライセンス状況 画面](images/3-7-license.png){#fig:license01 width=800px}  

	構成に合わせて必要なライセンスのみが提供されます。各ライセンスの説明は[@tbl:riyouLicense] の通りです。  

	No. | ライセンス名 | 説明
	--:|-----------------|---------------------------|
	1 | AmiVoice Communication Suite Core |Core ライセンスです。<br>算出根拠：AmiVoiceCommunicationSuite データベース毎に1ライセンス必要です。  |  
	2 | AmiVoice OperatorAgent サーバ版 | サーバ版で OperatorAgent を利用するライセンスです。<br>算出根拠：OperatorAgent をインストールするPCの台数分必要です。<br>※仮想環境で利用する場合は<br>ライセンスのカウント方式を **インストール台数** から **同時起動数** に変更できます。<br>仮想環境算出根拠：OperatorAgent の同時起動数分。（最低限 No.3 のRealTimeRecorder のライセンス数分はライセンスを購入して頂く必要があります）
	3 | AmiVoice RealTimeRecorder |サーバ版で録音するためのライセンスです。<br>算出根拠：録音対象としてシステムに登録する内線番号の数だけ必要です。
	4 | AmiVoice StreamingRecognizer |サーバ版で音声認識するためのライセンスです。<br>算出根拠：同時認識する数だけ必要です。（最大同時通話数と合わせるのがベターです。）
	5 | AmiVoice OperatorAgent コンバージャ版 |クライアント版で OperatorAgent を利用するライセンスです。本ライセンスに録音認識するためのライセンスが含まれています。<br>算出根拠：OperatorAgent をインストールするPCの台数分必要です。
	6 | AmiVoice MediaScriber | MediaScriber で音声認識するためのライセンスです。<br>本ライセンスに録音認識するためのライセンスが含まれています。<br>算出根拠：認識完了までの時間（要件）により、ライセンス数が変わります。<br> 計算式：最大録音時間 ÷ 認識処理実施時間

	: Communication Suiteライセンス一覧 {#tbl:riyouLicense}

- ライセンスの使用  
提供されたライセンスが使用されるタイミングは以下です。

	No. | ライセンス名 |ライセンスの使用タイミング
	--:|-----------------|---------------------------|
	1 | AmiVoice Communication Suite Core |なし（管理上のライセンスとなります。）  |  
	2 | AmiVoice OperatorAgent サーバ版 |OperatorAgent の初回ログイン処理を実施したタイミングでPC 毎に1つ使用<br>仮想環境の場合はライセンス使用を同時ログイン処理毎に変更可|  
	...
	![](images/Check.png){width=50px}　使用ライセンス数が総ライセンス数に到達した場合、以降はライセンス不足となり次の影響を受けます。  
	- No.2 No.5 は 「クライアントライセンス管理」に登録されているマシン（PC）以外で OperatorAgent にログイン不可
	- No.3 は内線番号の追加不可
	- No.4 No.6 は各認識の最大同時処理数の増加不可  
	ライセンス不足を解消するには、No.3,No.4,No.6 は設定値を減らす、No.2,No.5は登録済みの PC を減らす対処が必要です。  
<br>
- CommunicationSuite ライセンスの登録  
	ライセンスの登録にはライセンスファイルが必要です。  
	TaskRunner の`LicenseRegister` コマンドを実行してチャレンジコードを発行します。  
	発行されたチャレンジコードに対応するワンタイムパスワードを専用 IVR より取得してアクティベーションします。  
	- ワンタイムパスワード専用 IVR  
	連絡先：050-3160-0203  
...
- 感情解析ライセンス  
	...
	感情解析ライセンスは ContronCenter / システム管理 / ライセンス状況 /感情解析ライセンス管理 から確認できます。（[@fig:kanjo]）  

	![感情解析ライセンス管理 画面](images/2-1-感情ライセンス.png){#fig:kanjo width=900px}  

  - 購入が必要なライセンス数  
	感情解析を利用するにあたり、最低限購入が必要なライセンス数は以下です。  
	リアルタイム版・・・ RealTimeRecorder ライセンスの数  
	バッチ版・・・  MediaScriber ライセンスの数  
<br>
  - 感情解析可能な通話数  
	購入ライセンス数 × 3000 が感情解析可能な通話数としてシステム反映されます。（バッチ版は例外あり。後述参照）   
	リアルタイム版・・・購入したライセンス数 × 3000 回  
	バッチ版・・・購入した **最初の1ライセンスのみ 30,000 回** 。以降1ライセンス追加ごとに 3,000 ずつ追加
。  
	![](images/Check.png){width=50px}　感情解析ライセンスは月単位の利用形態となり、これは月ごとの感情解析利用回数です。ライセンスの利用状況は （[@fig:kanjo]） の`今月の感情解析通話数`より確認できます。消費した感情解析通話数は毎月1日の午前9時にリセットされて「0」に戻ります。利用可能なライセンス数を超過した場合には翌月のライセンスリセットまで、それ以降の通話の感情解析が実行されません。  
<br>
  - 今月の感情解析通話数の消費タイミング  
	ライセンスは **StreamingRecgnizer の認識処理実行時** に消費されます。(例外は後述 Tip 参照)  
	ライセンス消費数は、録音構成（通話プロバイダ、キャプチャポイント）や通話パターンの組み合わせにより変わりますが  
	基本的なライセンス消費の考え方としては **通話単位ごとにライセンスが1つ消費** されます。  
	保留イベントを検知できない場合には会話相手ごとにライセンスが消費されると考えてください。  
<br>
	感情解析ライセンスの消費例  
	1. 保留操作時の通話パターン  
   		- 保留イベントを取得できるパターン  
	![](images/3-7-kanjoz4.png){#fig:kanjoz4 width=650px}  

   		- 保留イベントを取得できないパターン  
	![](images/3-7-kanjoz2.png){#fig:kanjoz2 width=650px}  

   		- 保留イベントを取得できないが、保留影響を受けないパターン（コンバージャや RTP 録音時）  
![](images/3-7-kanjoz3.png){#fig:kanjoz3 width=650px}  

	1. 転送時の通話パターン  
	転送時の通話パターンも保留イベントの取得可否で消費数が変わります。  

   		- 保留イベントを取得できるパターン  
	![](images/3-7-kanjoz1.png){#fig:kanjoz1 width=1000px}  

   		- 保留イベントを取得できないパターン  
	![](images/3-7-kanjoz5.png){#fig:kanjoz5 width=650px}  

   		- 保留イベントを取得できないが、保留影響を受けないパターン（コンバージャや RTP 録音時）  
	![](images/3-7-kanjoz6.png){#fig:kanjoz6 width=650px}  


	![](images/Check.png){width=50px}　録音構成や通話パターンに関係なく、ライセンスを消費するケースもあります。  
	- 「1通話の最大録音時間」機能により通話が複数に分割された場合
	- 　再認識した通話（認識オプションの「感情解析が未処理の場合のみ有効」ではなく、「有効」の場合）
	- 「短い通話と判断する通話時間」機能によって結果的に通話は作成されずキャンセルされたが、認識処理が行われた場合

	![](images/Tips.jpg){width=50px}　ライセンス消費の例外として認識オプションの「感情解析で無視する冒頭区間」の範囲内で通話が終了した場合には感情解析ライセンスは使用されません。（[3-5-4. 感情解析の「感情解析で無視する冒頭区間」を参照](#3-5-4. 感情解析参照)）  
<br>
- 感情解析ライセンスの登録  
...
### 3-8. レポート
CommunicationSuite のすべてのコンポーネントに関するログイン履歴と通話の操作履歴を表示します。

#### 3-8-1. ログイン履歴一覧  

- 保存先  
	CommunicationSuite データベースのデータを保存しているファイル（Data.mdf, Index.ndf, LargeObject.ndf）に保存されます。  
	どれか１つのデータベースファイルのみに保存されるわけではありません。  
- 保存期間  
  削除されることはありません。  
...
- 保存先  
	ログイン履歴と同様です。  
- 保存期間  
	TaskRunner の削除処理で対象通話が削除されると操作履歴も削除されます。  
...
##### サーバパケットキャプチャ
1. Avaya  
Avaya 電話基盤の通話を録音できるようになります。  
電話基盤から取得できる通話属性情報がほぼ無いため、通話データの検索性を重要視する利用用途の場合には向きません。  
1. Avaya AES  
	- TSAPI ライセンス  
この通話プロバイダを利用する場合、内線番号管理に登録されたデータ数分の TSAPI ライセンスが必要になります。  
	- 通話属性 エージェント情報とエージェントステータス  
通話データの属性としてエージェント情報を付加し、更にそのエージェントステータスを座席表上で管理したい場合、[@tbl:amdl] の項目に、ダミースキルを設定する必要があります。  
ダミースキルとは、システムを利用する可能性があるエージェント全てを網羅するスキルです。  

		No. | 設定分類 | 設定項目名       | デフォルト値 |特記事項
		---:|------------------|--------------|------|--
		1 | RealTimeRecorder - AVAYA AES 連携 | 追加でモニタするデバイス一覧 | - | ここに設定したデバイスは1つに付き、1 TSAPI ライセンスを消費します。  

		: エージェント情報の取得 {#tbl:amdl}

	- 待ち呼と放棄呼  
スキル毎に、待ち呼と放棄呼の情報を取得することができます。  
取得する場合、[@tbl:amdl] に VDN を登録する必要があります。  
設定・利用している VDN のうちで、トランクから直結しているもの全てを網羅してください。  
IVR から直接呼ばれるものも対象にするか？などは電話基盤の設計・導入ベンダーに確認してください。  

	- 送信キューサイズ  
[@tbl:sendqueue] の 「送信キューサイズ」(sendQSize) は、TSAPI が API 呼び出しをサーバに送信するまでローカルで貯めておくキューで、単位は API の呼び出し回数をそのまま表しています。  
送信キューがフルの状態で、新たな API 呼び出すとキューイングに失敗し、該当の API 呼び出しそのものが失敗してしまいます。  
RealTimeRecorder と AES 間の NW 速度が遅く、内線番号管理の登録数が多い大規模環境では、発生しやすくなります。  
送信キューサイズの適正値は、内線番号管理のデータ数の 2 ～ 3 倍となります。  
（設定値が大きいことに関してのデメリットは現時点で確認できていません。）  

		No. | 設定分類 | 設定項目名       | デフォルト値 |特記事項
		---:|------------------|--------------|------|--
		1 | RealTimeRecorder - AVAYA AES 連携 | 送信キューサイズ | 0 |  

		: TSAPI 送信キューサイズ {#tbl:sendqueue}

1. Avaya APC  
Avaya Proactive Contact のイベントドリブンで、録音を制御します。  
【要確認】 APC 正式対応 3.7 の仕様が明らかになってから追記する  
1. SIP  
SIP 電話基盤の通話を録音できるようになります。  
	- TCP SIP に対応しています。  
	- SIP パケットのフラグメントに対応していません。  
	- 他の SIP と比較して若干クセが違う電話基盤  
		- SV9500  
		保留のイベントの取得ができません。（保留開始時に通話録音が一旦停止し、保留解除時に別の通話として録音されます。）  
		- CTstage  
		デフォルト値から変更の必要のある設定は、[@tbl:cts] を参照してください。  
		ただし、ソフトフォン用のデータ系の SIP では以下設定がデフォルトのまま取得できる可能性があります。  

			No. | 設定分類 | 設定項目名       | デフォルト値 | CTstage の場合
			---:|------------------|--------------|------|--
			1 | RealTimeRecorder - SIP パケットキャプチャ | SIP のインバウンド時の着信番号の取得 | 0 | 1  
			2 | RealTimeRecorder - SIP パケットキャプチャ | 通話基本情報の確定タイミング | 0 | 1  

			: CTStage の SIP {#tbl:cts}

1. SIP CTstage連携  
	- 追加の設定など  
連携用の CTstage クライアントは、手動で追加インストールする必要があります。  
【要確認】 手順やクライアントインストール時の注意事項  
1. SIP T-Server連携  
	- 利用目的  
SV 9500 と T-Server を組みわせて、保留時に通話が切断されないようにするための通話プロバイダです。  
	- 追加の設定など  
		1. 連携用の各種 DLL は、手動でコピーする必要があります。  
		1. 保留イベントを取得するには、ノード管理の RealTimeRecorder ノード設定の 『オプション』 に `HoldJoin=true` を設定する必要があります。  
	- 利用における注意点  
通話録音中に、SIP イベントよりも早いタイミングで、当該イベントに対応する T-Server イベントを受信してしまうと、以降当該通話には、T-Server イベントが紐付かなくなってしまうことがある事象が確認されています。  
現在のバージョンでは、本プロバイダよりもはるかに安定した 『SIP OAI連携』 が存在するため、非推奨となっています。  
1. SIP OAI連携  
SV 9500 利用下で保留時に通話が切断されないようにするための通話プロバイダです。  
T-Server 連携との大きな違いは、RealTimeRecorder から直接 OAI と通信している点です。  
	- 追加の設定など  
		1. 連携用の各種 DLL は、手動でコピーする必要があります。  
【要確認】 手順や注意事項  
	- PBX 側への要求仕様  
		1. `ASYD SYS1 INDEX241 ビット7 = 1`  
		1. `ASYDL SYS1 INDEX877 ビット4 = 1`  
	- 利用における注意点  
OAI の利用目的が通話や電話機の状態を把握するもののため、要約・分析外部システムとの連携が必須のコンタクセンターシステムに組み込む場合、必要な情報は他システム（CRM など）から補完してもらう必要があります。  
1. SIP CIC連携  
Genesys 社の PureConnect （旧 ININ 社、CIC） 電話基盤利用下で、同 PBX が公開している API 『IceLib』 と連携して、各種通話属性を追加取得するための通話プロバイダです。  
	- 追加の設定など  
		1. 連携用の各種 DLL は、手動でコピーする必要があります。  
		1. 下記の注意点の記載内容に伴い、OS のパッチをインストールしたり、アンインストールしたり等の対応が必要になることがあります。  
	- 利用における注意点  
正常動作のための環境条件が非常にシビアで、動作しない場合のシューティングが、Communication Suite 側からだけでは困難を極めます。  
電話基盤導入ベンダ及び電話基盤メーカーの柔軟な協力体制が存在しない場合には利用しないことを推奨します。  
以下の3点が、キーポイントです。  
		1. PBX のバージョン及びパッチレベル  
		1. RealTimeRecorder 側の IceLib 関連の DLL のバージョン  
		1. RealTimeRecorder サーバの OS のパッチレベル  
特定の KB の適用状況で、動作したりしなかったりします。  
1. RTP  
...
1. RTP AES  
録音の開始終了・保留開始解除など、通話イベント発生のトリガーに、H.323 を利用しない録音方式です。  
代わりに、AES から通知される通話イベントをトリガーに録音をコントロールします。  
...
##### クライアントパケットキャプチャ
手法としては、SLC を利用したハードフォンタイプ、SLC を利用しないソフトフォンタイプが存在します。  
プロバイダは、  
...
- 利用の注意点  
	1. CTI 連携系通話プロバイダは存在しません。
	1. SLC タイプの場合、USB LAN アダプタも、SLC 同様に弊社から出荷された製品のみをご利用ください。  同じ型番でないと動作しません。  
...
RealTimeRecorder のノード設定の拡張設定を有効にすると以下の効果を得られます。  

![](images/Check.png){width=50px}　拡張設定は、サーバパケットキャプチャ系の通話プロバイダにしか効果がありません。（AmazonConnect 連携利用時には無効な設定なので注意してください。）  

1. 呼制御通信（SIP や 323）と音声通信（RTP） が1 ノードの異なるキャプチャ I/F から受信するケースでも正常録音を担保できるようになります。  
	- 1通話の呼制御 (SIP や H.323) が複数のキャプチャ I/F に分かれて受信するケースでは **正常録音を担保できません**。  
![](images/NOTICE.png){width=50px}　本効果が目的の場合、拡張設定を有効化するよりも、NIC をチーミングして対応する方法を推奨します。  

1. RealTimeRecorder の処理能力を効率化し、大規模の同時録音に対応できるようにする。  
