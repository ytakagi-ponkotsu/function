...
1. 通話内容  
OperatorAgentでログインした内線番号で通話が開始されるとControlCenterより状態通知を受け取り   
通話開始を検知します。通話開始後、認識結果をStreamingRecognizerから取得してオペレータとカスタマの   
発話内容を画面上にテキスト表示します。 ![2-1-通話内容](images/2-1-通話内容.png)
 ...
 **※** @_数字_モジュール_処理名.vbs のファイルは、自動アップデート時に特定の処理を行う (不要になったファイルの削除や、不具合修正のための処理とか) 為にあるので、以降のバージョンで追加になる可能性があります。
