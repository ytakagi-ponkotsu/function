...
-	通話属性を追加する場合の書式は  「通話属性識別名=名称」となります。

	例） 検索結果に表示する通話属性に「ダイヤルイン番号」を表示する

	```
	amivoice.common.telephony.called.phonenumber=ダイヤルイン番号
	```

-	通話属性識別名が「〇〇.url」のものは通話属性値を使って  [@fig:local1] のように外部ページへのリンク表示が可能です。  
	例） 検索結果に表示する通話属性に「ローカル参照用のURL」を表示する。

	```
	amivoice.common.reference.local.url|label=ローカル参照用のURL
	```


	![外部ページへのリンク表示](images/2-3-通話属性ローカル参照.png){#fig:local1 width=600px}

-	通話属性値をリンク先のURLに含めることも可能です。  
	「 通話属性識別名 | label=ラベル | url=リンク先URL | urlLabel=リンクのラベル 」が書式となります。  
	` url=`のリンク先URL に `${value} ` を指定することで通話属性値をリンクのラベルに含めることができます。  
	例）相手番号の通話属性値をURLに含める

	```
	amivoice.common.customer.phonenumber|label=相手番号|url=http://host/CustomerPage.aspx?key=${value}|urlLabel=CRM ページへ
	```

	[@fig:link1] は相手番号の通話属性値をURLに含めてリンク表示した結果です。  

	![相手番号の属性値にURLを含めた結果](images/2-3-通話属性リンク.png){#fig:link1 width=800px}
	...
	検索結果に 〇 が付いているものはデフォルト設定で表示される通話属性です。  
...
<br>
 4.  検索結果の認識内容  
	検索結果に表示される認識内容は不要分を除外して表示しています。  
		...
		![実際の認識結果](images/2-3-検索結果6.png){#fig:ninsikikekka2 width=800px}  
...
1. 詳細設定項目（[@tbl:detaildc])
	...
	18 | SpeechVisualizer - 通話詳細 |通話詳細に表示する感情解析階調線の数 |3 |
...
- 通話詳細に表示される感情解析結果
	...
	```
	感情解析項目識別名|color=色を表す6桁の16進数|upperLabel=名称(高)|lowerLabel=名称(低)|border=線の太さ|symbol=点の形(circle, square)
...
<font color="#fa0000">『リアルタイム認識』</font> の設定値に従って処理されます。（StreamingRecognizer ノードの 『バッチ認識 - 最大同時認識数』 の **上限を越えて動作します。**）  
	...
	- 認識中（B）\_疑  
リアルタイム認識であっても、録音完了前には認識処理が終了することはありませんので、この状態が異常であると断定することはできません。  
ただし、正常なパフォーマンスがキープされていてれば、通話終了後 5 秒以上の間、認識中（B）\_疑 の状態が継続することは考えにくいです。  
...
1. 各ステータスでの滞留の原因と対処
	- 認識前  
＃＃＃＃＃＃＃＃＃＃＃＃＃＃＃この内容は認識前じゃないな。どこだっけな。
		- 滞留原因  
リアルタイム 構成では、RR からの通話開始アップロード処理が DB のデッドロック頻発により DB パフォーマンスが低下することで、大幅に遅延し滞留が発生することがあります。  
Ver 3.2 未満でかつ、規模が大きい事例（同時 200通話以上）で顕著に発生します。  
		- 発生事象  
通話の開始処理の登録が遅延し、"認識前"ことから、その後の ControlCenter が未認識のキューを
リアルタイム認識処理がシステム全体で遅延していきます。最終的にはほぼ全ての認識がリアルタイムで処理されなくなります。
ノード管理画面上では、認識中（<font color="red">R</font>） の数値がカウントアップされなくなり、システム全体の認識キューがバッチ認識（認識中（<font color="blue">B</font>））で処理されていきます。  
この場合のバッチ認識には、認識中（B）\_疑 の通話も含まれます。
		- 解消方法  
SR のプロセスを部分的に停止します。どこかのタイミングで滞留数値が下がり始めます。  

	- 認識待機中  
認識待機中（<font color="blue">B</font>） がカウントアップされる状態で想定の動作となります。（設計値にもよりますが。）  
認識待機中（<font color="red">R</font>） がカウントアップされる場合、  
		1. 同時認識ライセンス数  
		1. StreamingRecognizer ノードの 『リアルタイム認識 - 最大同時認識数』 の設定値  
		1. StreamingRecognizer ノードの HW スペック  
