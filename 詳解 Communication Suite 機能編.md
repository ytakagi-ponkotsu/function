...
- 検索モジュールのプロバイダは、ControlCenter 詳細設定の [@tbl:provider0] の項目を編集して構成します。  
	...
	- 検索結果の並び順の設定  
		[@fig:narabijun] は検索結果のソート条件で利用する検索クエリです。  
		...
		No. | 設定項目名       | 設定値 | 内容 |
		---:|-----------|----------|-------------------------|
		1   |  SpeechVisualizer - 通話検索  | 検索で使用する並び順        | 検索で使用する並び順の検索クエリのリストです。表示する検索クエリを改行で区切ります。 |
...
通話検索画面の UI で検索条件として指定する検索クエリの仕様について説明していきます。  
   ...
   1. 基本のクエリ  
		 ...
		 ```
		 - 条件の組み合わせ  
		 検索クエリを複数組み合わせて AND 検索 することができます。その場合には基本のクエリをスペース区切りで連結します。  

		 	```
		 	<検索クエリ1>:<条件1><スペース><検索クエリ2>:<条件2>
		 	```
		 ...
		 - 検索条件のオプション  
			 ...
			 No. | オプション     |説明
			 ---:|-----------------|----------|
			 1   |\,  [カンマ]|  １つの検索クエリの条件内で複数条件を指定する
			 2   |\-  [ハイフン] | ・1つの検索クエリの条件内で指定したものを除外する<br>・日付や時間で使用する場合は範囲指定となります  

			: 検索条件のオプション {#tbl:option1}

      		- カンマ  
					１つの検索クエリの条件内で複数条件を指定する場合に利用します。

				```
				<検索クエリ>:<条件1>,<条件2>
				```
				例）インバウンドとアウトバウンドの2つのプロジェクトを指定
 ...
 3. フリーワード検索  
	フリーワード検索は検索クエリが不要です。認識結果に含まれる任意の単語を直接検索条件に入力します。   
	例） 「りんご」の単語が含まれる通話を検索
	...
	- 検索のオプション  
	フリーワード検索では論理和（OR 検索）、論理積（AND 検索）、論理否定（NOT 検索）の3つを指定することができます。  
	[@tbl:option2] はフリーワード検索で利用可能なオプションです。  

		No. | オプション     |説明
		-:|-----|--------------------|
		1   |or|論理和 （OR 検索）： 指定した2つ以上のフリーワードのいずれかを条件とする   
		2   |空白 [スペース]|論理積 （AND 検索）：指定した2つ以上のフリーワードを条件とする  
		3   |- [ハイフン]|論理否定 （NOT 検索）：指定したフリーワードを除外します。<br>NOT検索のみで指定した場合、すべての検索単語が対象となるため、検索が無視されます。除外対象と検索対象とする単語の組み合わせで利用する必要があります。<br>
		...
		- 論理積（AND 検索）  
			...
			```
			<フリーワード1><スペース><フリーワード2>
			```
			例）「りんご」と「みかん」の単語が含む通話を検索  
 ...
 3. フリーワード検索の仕様  
	...
	- フリーワード検索の動作条件  
 CommSuite データベース稼働サーバで「SQL Full-text Filter Daemon Launcher (MSSQLSERVER)」サービスの起動が必要。  

	- フリーワードの対象  
	フリーワードの対象となる文字列は SQL Server 機能のワードブレーカーによって分割された単位が対象。  

	例）認識結果  

	```
	お電話ありがとうございます。コールセンターのヤマダです。
	```
	...
	3   |ルセンター|×|ワードブレーカーが区切った単語と一致しないため

	: フリーワード検索の結果 {#tbl:option3}
