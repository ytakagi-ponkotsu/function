...
#### 2-4-1. 通話詳細の画面構成
- 通話詳細は『メインビュー』と『通話情報ビュー』で構成されています。各通話の通話詳細はRealtimeRecoderからDBへの通話情報のアップロード完了後、アクセス可能となります。（[@fig:calldetail])【要確認】簡易確認では記載の認識だが、複数ある？もしくは異なる？
...
1. 基本タブの項目について
	...
	基本タブは、通話属性が付与されている通話の場合、[1-2-2. 通話表示機能](#1-2-2. 通話表示機能) の [@tbl:callb2] の 『通話詳細』列の〇の内容が『名称』列を項目名としてデフォルトで表示されます。項目名については[@tbl:detaildc] の『通話情報に表示する追加の通話属性』から変更が可能です。『通話情報に表示する追加の通話属性』の設定書式は以下の通りです。
