...
オレンジの吹出は、録音対象の電話機を主体として受話音声の認識結果となります。受話側音声の認識は、認識オプション設定のカスタマタブに設定されたエンジンモードで処理されます。（[@fig:txengine]）  
...
　このケースは通話録音・認識は正常に完了しているが、 <br />　　**OperatorAgent と StreamingRecognizer 間の通信の不備** の可能性が疑われます。  
