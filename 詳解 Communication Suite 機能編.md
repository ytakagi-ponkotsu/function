...
　　１内線あたり、月 3,000 通話としてライセンス処理可能な契約となっています。  
　　例） 5 内線分のライセンス環境の場合  
...
　　１つの通話でも、以下のような条件下では感情解析ライセンスが消費されます。  
...
- 感情解析の対象範囲  
  ...
  ----:|---------------------|------------------|--------------|--------------|
  1   |オペレータ / カスタマ | 感情解析で無視する冒頭区間 |ミリ秒単位で設定 |感情解析の対象外とする冒頭区間を指定します。この区間内に通話終了した場合は感情解析ライセンスが消費されません。  
...
手動で通話属性を追加する項目は ConttrolCenter の詳細設定項目にあります。
...
追加可能な通話属性は（[1-2-2. 通話表示機能　表 11: OperatorAgent 表示する通話属性で利用可能な通話属性一覧](#1-2-2. 通話表示機能) 参照。）  
