...
- 認識テキストが表示されない。
	...
	No. | 原因| 切り分けと対処 |
	----:|---------|-------|
	1 | 音声認識エンジンが設定もしくは適用されていない。  | ControlCenterの「認識オプションの設定 - システムデフォルト」を確認して、「テキスト認識」が「無効」もしくは「音声認識エンジンモード」が「未設定」の場合、テキスト認識が行われません。設定および「ノード管理」から設定の再読み込みを実施したのち、通話の再認識を実施してください。   |
	2 | DBの容量が不足している。  | ControlCenterの「ノード管理」を確認して、「認識完了」に通話が残存している場合に該当します。SQLServerManegementStdioを使用してDBの使用量を確認し、領域の拡張と必要であればHDDの拡張をお願いいたします。  |
	3 | StreamingRecognizerからのアップロードに失敗している。  |  ControlCenterの「ノード管理」を確認して、「認識エラー」に通話が残存している場合に該当します。  サーバ障害等の|   |   |   |
