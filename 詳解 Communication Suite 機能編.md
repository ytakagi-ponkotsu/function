...
- 検索モジュールのプロバイダは、ControlCenter 詳細設定の [@tbl:provider0] の項目を編集して構成します。  
	...
	表 64 で補足します。
		...
		2   |  attributes | 通話属性識別名        | 文字列 | 通話検索条件に追加する通話属性識別子です。通話属性識別名は CommunicationSuite 内で通話属性毎に設定されているユニークな識別名です。使用可能な通話属性については（[1-2-2.　[@tbl:callb2] : 利用可能な通話属性一覧](#1-2-2. 通話表示機能) 参照。）  を参照してください。【要確認】追加できる通話属性とそうでないもの  | ×|
...
#### 2-4-1. 通話詳細の画面構成
- 通話詳細は『通話内容ビュー』と『通話情報ビュー』で構成されています。各通話の通話詳細はRealtimeRecoderからDBへの通話情報のアップロード完了後、アクセス可能となります。（[@fig:calldetail])【要確認】簡易確認では記載の認識だが、複数ある？もしくは異なる？

	![通話詳細](images/2-4_detail.png){#fig:calldetail width=600px}  

	通話詳細画面のURLは、引数に会話識別子( ConversationKey )を指定する必要があります。
...
1. ユーザ毎にカスタマイズが可能な設定  
	...
	![](images/Tips.jpg){width=50px}　詳細設定で指定する場合も、通話詳細画面の UI からカスタマイズする場合でも、表示・非表示を切り替える項目は、利用を制限するための機能としては利用できません。  
...
1. 通話内容ビューでの通話イベント表示
	...
	4 | 保留解除|保留解除|
	5 | 保留終了|保留終了|
	6 | 転送完了|保留終了|転送関連情報が連携される一部API連携時のみ利用される。【要確認】：条件<br/>転送完了と同時に通話が終了した場合は通話終了が省略される
	7 | 保留エラー|保留エラー|
	:Speechvisualizerでの 通話イベント {#tbl:detailevent}

	![](images/Tips.jpg){width=50px}　通話イベントはそのイベントが取得できる場合のみ表示されます。例えば、MediaScriber版での音声取り込み方式では通話状態が判断できないため、『通話開始』『通話終了』のイベント以外は表示されません。

1. 通話内容ビューに表示される感情

	通話内容ビューにはオペレータとカスタマの感情の時系列折れ線グラフ、シークバーに感情の時系列階調線が表示されています。
...
- 音声が再生できない
	...
	----:|---------|-------|
	1 | wmaファイルにアクセスできない |  
	2 | セキュアな通信下でSilverlight利用の再生をしている |  
	3 | 非対応のブラウザを利用している |  
	4 | WindowsMediaPlayerが有効になっていない。  |  

	: 音声再生のトラブルシューティング {#tbl:audioplaytrouble}

	![](images/Check.png){width=50px}　『Silverlight プラグインの利用 』が『False』もしくはブラウザの『Microsoft Silverlight』アドオンが無効化されている場合、通話詳細画面へ遷移後、初回の音声再生は**ダウンロードが完了した後に通話の冒頭から再生されます**。途中再生を選択していても必ず冒頭から再生されます。
...
1. 基本タブの項目について
	...
	任意の通話属性が付与されている通話の場合、[1-2-2. 通話表示機能](#1-2-2. 通話表示機能) の [@tbl:callb2] の 『通話詳細』列の〇の内容が項目としてデフォルトで表示されます。項目名については[@tbl:detaildc] の『通話情報に表示する追加の通話属性』から変更が可能です。『通話情報に表示する追加の通話属性』の設定書式は以下の通りです。
...
``"通話属性"="項目名"``の形式で改行区切りで記載します。
	...
	![](images/Tips.jpg){width=50px}　 [@tbl:callb2] の 『通話詳細』列で〇が付いていないものも追加可能です。ただし、『通話詳細』に〇が付いていない項目は通話詳細上では設定が推奨されない、もしくは設定しても使用されない項目になります。

	![](images/Tips.jpg){width=50px}　 項目『録音サーバ』および『認識サーバ』には、それぞれ録音および認識で使用されたサーバノードの『ノード識別名』が記載されます。通話に紐づいた調査やログ回収を実施する場合には、この項目を確認することでどの録音・認識プロセスが使用されたか確認することができます。

1. 認識タブからの再認識

	通話の再認識の実行は条件を満たした一部の通話にのみ実行できます。
	[@tbl:detailcallrecognition]は再認識に関連する設定です。

	No. | 設定分類 | 設定項目名       | デフォルト値 |特記事項
	---:|------------------|--------------|------|--
	1 | RealTimeRecorder - 録音音声 | データ削除条件 |0 |
	2 | RealTimeRecorder - 録音音声 | データ保存期間 |7 |
	: 再認識用音声の設定 {#tbl:detailcallrecognition}

	『データの削除条件』を満たしておらず、『データの保存期間』内で、実行ユーザに『再認識の登録』権限付与されていれば再認識の実行が可能です。
...
RealTimeRecorder 側の録音エラー（1-2 へのリンク）や、RealTimeRecorder のクラッシュも含みます。  
...
- 感情解析ライセンス数の算出式
	...
	※ クライアント版は OperatorAgent ライセンス が対象

	- MediaScriber の場合  
	MediaScriber ライセンス数 1 につき、30,000 回の感情解析処理 / 月で算出されます。  
	※ はじめの1ライセンス以降は1ライセンス追加ごとに 3,000 ずつ追加されます。  
<br>
- 感情解析ライセンスの消費タイミング  
	![](images/Tips.jpg){width=50px}　録音開始した通話を StreamingRecognizer が認識開始したタイミングで 1 通話分のライセンスとして１つ消費されます。ライセンス消費における 1通話は Communication Suite の1通話（通話検索で表示された通話件数）と同じです。  
<br>
	感情解析ライセンスが消費されるケースとして以下です。  
	- 保留により通話が分割される場合（通話プロバイダによっては保留で通話は分割されません）
	- 「1通話の最大録音時間」機能により通話が複数に分割された場合
	- 再認識を実施した通話（認識オプションで「感情解析が未処理の場合のみ有効」ではなく、「有効」の場合のみ）
	- 「短い通話と判断する通話時間」機能によって結果的に通話としては作成されずキャンセルされたが、認識処理が行われた場合  
<br>
	**ライセンス消費の例外として認識オプションの「感情解析で無視する冒頭区間」の範囲内で通話が終了した場合には  
	感情解析ライセンスは消費されません。**  
	...
	: ContolCenter 詳細設定の感情解析ライセンス消費に関連する設定 {#tbl:kanjouLicense1}  

- ライセンスの登録  
	感情解析システムIDを発行しアクティベーションコード入手後、[@fig:kanjo] 感情解析ライセンス画面の「アクティベート」ボタンよりアクティベーションコードを入力して感情解析ライセンスを登録します。感情解析ライセンスは ControlCenter をインストールするWEBサーバごとに登録が必要です。ロードバランサなどでWEBサーバを冗長化する場合には、それぞれのWEBサーバに感情解析ライセンスを半分ずつに分けて登録します。  

	![](images/Tips.jpg){width=50px}　アクティベーション登録直後は [@fig:kanjo] の状態欄が時計マークの「アクティベーション待ち」となり  
	その後の新規通話で感情解析が行われた時に「アクティベーション済」となり使用ライセンス数が表示されます。  

	登録時の用途によって用意されるアクティベーションコードのフォーマットが異なります。  
