...
1. 通話内容  
OperatorAgent でログインした内線番号で通話が開始すると通話内容の枠に「通話開始」の帯が表示されます。   
 ...
 **・対象通話の録音ができていない**   
 ...
 オペレータ側の発話は緑色枠に表示、カスタマ側の発話は橙色枠に表示されます。
 ...
 No. | 設定タブ項目 | 設定項目名                | 設定値      |
 ----|---------------------|------------------|--------------|
 1   | オペレータ | 音声認識用エンジンモード | AMI提供のオペレータ用音声認識エンジン |
 2   | カスタマ | 音声認識用エンジンモード | AMI提供のカスタマ用音声認識エンジン |
...
2. 通話情報   
 ...
 通話属性が取得できる場合に通話情報の欄に以下の属性情報を表示します。   
 自分の電話番号・・・自番号  
 自分のID・・・エージェントID   
...
3. 通話相手  
 ...
 2   |カスタマタブ | 性別識別用エンジンモード | 性別識別用エンジンを登録する
 3   |カスタマタブ | 性別識別の閾値| あああああ   
 4   |カスタマタブ | 性別識別に利用する発話時間（最大） | あああああ   
 5   |カスタマタブ | 性別識別に利用する発話時間（最小） | あああああ
...
4. 通話状態   
 通話相手の画面に表示される通話状態が表示されます。   

 ![2-1-通話状態](images/2-1-通話状態.png)   

 `利用のヒント`  
 通話が終了しているのに通話時間が継続してしまう場合には以下の原因が考えられます。   
 **・ControlCenter から通話終了の状態通知を OperatorAgent が受信できていない**   

5. 通話フィルタ   
設定したキーワードがテキスト化された場合に様々なアクションを実行できる機能です。   
 ...
 6. ヘルプ    
OperatorAgent から SpeechVisualizer の座席表にアラート通知する機能です。   
 ![2-1-ヘルプ](images/2-1-ヘルプ.png)   

 ヘルプを利用するには ControlCenter/モニタリング/ヘルプ要求理由管理とヘルプ要求解除理由にテンプレートの登録が必要です。   
 ヘルプの ControlCenter の詳細設定項目は以下です。

 No. | 設定分類| 設定項目名                | 内容      |
----|---------------------|------------------|--------------|
1   |OperatorAgent - メッセージ | ヘルプ対応時にメッセージウインドウを自動的に開く | ヘルプ応答時にメッセージウインドウを自動で開く   
2   |OperatorAgent - 通知メッセージ | ヘルプの解除理由選択の表示時間|ヘルプ要求の自動解除後にヘルプ解除理由のテンプレートを表示する時間    
3   |OperatorAgent - 通知メッセージ | ヘルプを案内する感情解析の通話重要度の閾値 | あああ   
4   |OperatorAgent - 通知メッセージ  |ヘルプ中のクローズ | あああ
5   |OperatorAgent - 通知メッセージ  |ヘルプ中の表示 | ヘルプ要求時の通知メッセージを表示

 `利用のヒント`   
 OperatorAgent のヘルプ通知が SpeechVisualizerの座席表に通知されない場合には以下の原因が考えられます。   
  **ああああああああ**   
 7. 感情解析ポップアップ   
 オペレータとカスタマの通話中の感情をリアルタイムで数値化して表示する機能です。   
 この感情解析結果は StreamingRecognizer から取得して表示しています。  
...
No. | 設定分類| 設定項目名                | 内容      |
 ...
 本設定を実施していない感情は感情値が取得できず、感情解析ポップアップに感情が表示されません。   
 ...
 `利用のヒント`   
  **保存する感情スコアに設定した感情のみがデータベースに保存されます。   
  利用する感情はパフォーマンスの観点からプリセットの８つまでに抑えるようにしてください。**   
