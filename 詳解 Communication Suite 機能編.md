...
1. 通話内容  
OperatorAgentでログインした内線番号で通話が開始すると通話内容の枠に「通話開始」の帯が表示されます。   
この状態通知はWEBサーバ（ControlCentere）とクライアント端末（OperatorAgent）が８０番ポートを介して通信します。
![2-1-通話開始](images/2-1-通話開始.png)   

  `利用のヒント`   
「通話開始」の状態通知が表示されないと、通話内容の枠に発話内容のテキスト化は表示されません。   
状態通知が検知できない原因としては以下が考えられます。   
・WEBサーバの80番ポートが開放されていない   
・録音開始のトリガーとなるイベントがキャプチャサーバで取得できていない   

 通話開始後、StreamingRecognizerから認識結果を取得してオペレータとカスタマの認識結果を表示します。   
 オペレータ側の発話は緑色枠に表示されてカスタマ側の発話は橙色枠に表示されます。
 ![2-1-通話内容](images/2-1-通話内容.png)   

 オペレータとカスタマで使用する音声認識のエンジンが異なります。   
 ControlCenter - 認識オプションの設定でオペレータ、カスタマの音声認識エンジンを設定します。   
 No. | 設定タブ項目 | 設定項目名                | 設定値      |
 ----|---------------------|------------------|--------------|
 1   | オペレータ | 音声認識用エンジンモード | AMI提供オペレータ用エンジン |
 2   | カスタマ | 音声認識用エンジンモード | AMI提供カスタマ用エンジン |
 ...
 `よくお問い合わせを受ける事象`  
 通話開始・通話終了の帯は表示されるが、テキスト結果が画面上に表示されない   

 原因   
 ・クライアント端末からStreamingRecognizerのHTTPポートに接続が出来ていない   
 ・ノード管理 - ノード詳細のStreamingRecognizerのノードホスト名（IPアドレス)が正しくない
 ...
 通話の状態を帯で表示します。   
   ...
   No. | 帯の表示 | 意味|
   ----|---------------------|------------------|
1   | ![2-1-通話開始](images/2-1-通話開始.png) | 対象通話の開始 |
2   | ![2-1-通話終了](images/2-1-通話終了.png)| 対象通話の終了 |
3   | ![2-1-通話切替](images/2-1-通話切替.png)| 〇〇〇なにかく |
4   | ![2-1-通話終了](images/2-1-通話終了.png)|  |
5   | 外線通話のアイコン | 通話相手が外線電話の場合に表示 |
5   | 保留のアイコン | 通話中に内線側が保留した場合に表示|
6   | 会議のアイコン | 会議通話を利用時に表示|
7   | INのアイコン | 通話中に内線側が保留した場合に表示|
8   | ![2-1-OUT](images/2-1-OUT.png)| OperatorAgent - 通話全文コピー |
9   | ![2-1-通話切替](images/2-1-通話切替.png) | OperatorAgent - 通話全文コピー |
...
2. 通話情報   
   「自番号」の通話属性が取得できる場合に通話中のオペレータの内線番号が表示されます。   
