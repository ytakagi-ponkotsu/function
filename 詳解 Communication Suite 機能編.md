...
- 感情解析の対象範囲  
  １通話内で感情解析の対象とする範囲を設定することができます。  
  設定はControlCenter / 認識オプション にあります。


	No. | 設定タブ項目 | 設定項目名                | 設定内容      |説明      |
  ----:|---------------------|------------------|--------------|--------------|
  1   |オペレータ / カスタマ | 感情解析で無視する冒頭区間 |ミリ秒単位で設定 |感情解析の対象外とする冒頭区間を指定します。この区間内に通話が終了した場合には感情解析ライセンスが消費されません。アウトバウンドの業務通話を自動発信している場合などには留守電に繋がった場合でも感情解析ライセンスが消費されますので本設定を利用することで不要な通話を感情解析の対象外とすることが可能です。  
...
SpeechVisualizer の通話検索の検索条件に任意の通話属性を追加したり、既存の検索条件を削除したりカスタマイズが可能です。
#### 2-3-1. 検索モジュールの通話アイコン（プロバイダ）
通話検索に利用する通話アイコン([@fig:provider])を追加・削除することが可能です。  

![検索モジュールのアイコン](images/2-3-検索モジュールのプロバイダ.png){#fig:provider width=600px}  

検索モジュールの通話アイコンの設定は ControlCenter の詳細設定にあります。

No. | 設定項目名       | デフォルト値 | 内容 |
---:|------------------|--------------|------|
1   |  検索モジュールのプロバイダ  | [@tbl:provider] 参照        | 検索モジュールを提供するプロバイダのリストです。各プロバイダを改行で区切ります。 |
...
**新規で通話検索条件に通話属性を追加する場合は[@tbl:provider3]の書式に合わせて追加してください。**
...
各項目は「 | 」で区切ります。通話検索条件を複数追加する場合は、改行区切り接頭語以外は順不同です。  

**(注1):通話属性識別名**  
通話属性識別名は CommunicationSuite 内で通話属性毎に設定されているユニークな識別名です。  
使用可能な通話属性については（[1-2-2. 通話表示機能　表 15: OperatorAgent 表示する通話属性で利用可能な通話属性一覧](#1-2-2. 通話表示機能) 参照。）  を参照してください。  

**(注2):アイコン名**  

No. | アイコンの画像      | アイコンの種類| アイコン名   
---:|------------------|--------------|------|
1   |  【要確認】 | 人物        | person1|  
2   |  【要確認】 | 電話1       | tel1|  
3   |  【要確認】 | 電話2        | tel2|  
...
5   |  【要確認】 | 星1        | star1|  
6   |  【要確認】 | 星2       | star2|  
7   |  【要確認】 | 吹き出し1        | balloon1|  
8   |  【要確認】 | 吹き出し2       | balloon2|  
...
番号系の通話属性を通話検索条件に追加する場合は、検索プロバイダ "TelCondition" に下記属性を書式に合わせて追加してください。  

**書式**  
検索プロバイダ "TelCondition" の後に"|"を記述し、"targets=" の後に追加する属性を","区切りで列挙してください。  
明記しない場合(デフォルト)は "all*,op*,cu*,line" が設定されている状態になっています。属性を追加する場合は、上記値も追記してください。  
"Asterisk（記号）" を前か後ろに付けると、ワイルドカード扱いになり、前方一致、後方一致、中間一致になります。
...
座席表の通信は、以下の仕様で行われます。
 ...
 - 座席モジュールは、一つ一つが画面全体とは、独立して通信を行います。（すくしょ）  
 - 座席モジュールは、画面に表示されているときのみ、画面に表示されている情報量に応じて通信を行います。（すくしょ）  
 座席表をズームアウトすると、座席モジュールに表示できる情報は減ります。（非ピン留め状態では、テキストの遡りはできません。）  
 認識結果テキストも表示可能な情報分のみを通信し、テキストが表示されない大きさまでズームアウトした場合には、認識結果テキストの通信は行いません。  
 - 座席モジュールをピン留め状態にすると、継続している通話のテキスト全文を通信し取得します。  
 - アラートは、特に設定しない場合、座席モジュール上にのみ表示され、目視することで対応します。モニタリングリストを設定することにより、『通知リスト』 に対して発報できます。通知リストへは、画面表示範囲外の座席モジュールのアラート情報も通知されます。  
	...
	2   | 座席表 - 詳細 | ピン留め時にメッセージパネルを開く | false |   
