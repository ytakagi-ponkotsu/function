...
- ホーム画面のガジェット配置を設定できます。
	...
	![](images/NOTICE.png){width=50px}
	『ホーム画面のレイアウト (配置情報)』の設定で配置の対象となるのは『ホームモジュールのプロバイダ』で設定した『プロバイダ』のガジェットのみです。『プロバイダ』に追加済みでも『マイクエリ』ガジェットは SpeechVisualizer のマイクエリ機能、『通話スコアリング』、『通話品質評価テンプレート』、『スキル』ガジェットについてはそれぞれ ControlCenter で設定済みの場合のみ表示されます。

	<br />

	リンク以外のガジェットはプロパティでは、レイアウト時にガジェットの指定に続けて、`：`区切りでガジェットのプロパティを指定できます。
	ガジェット単位でプロパティ数が決まっています。プロパティはガジェットごとに、省略(デフォルト値を使用)もしくは全て記述する必要があります。
...
『Profile』はレイアウト時に以下の書式で、2つのプロパティについて定義することができます。
		...
		---:|------|------------------|-  |    
		1  | 設定なし  | 未設定  | このガジェットでは使用されません。【要確認】  |    
...
『MyRecent』はレイアウト時に以下の書式で、6つのプロパティについて定義することができます。
		...
		1  |表示件数の選択   |  5 | UI上は5,10,20件から選択、詳細設定では1~20件で設定可能です。  |    
		2  |自動更新の有効化   | 0  | 0:無効化,1:有効化   |   
		3  |設定なし   | 未設定  | このガジェットでは使用されません。【要確認】  
		4  |設定なし   | 未設定  | このガジェットでは使用されません。【要確認】    
		5  |設定なし   | 未設定  | このガジェットでは使用されません。【要確認】
...
- 通常検索の書式
	...
	```
	<プロジェクト検索プレフィックス><コロン><検索対象のプロジェクト>
	```

- 自分の所属プロジェクト検索の書式  

	```
	<自分の所属プロジェクト検索プレフィックス><コロン><検索対象の自分の所属プロジェクト>
	```
