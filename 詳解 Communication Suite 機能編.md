...
>>>>>>> 1973d685ff71355ec6c8e916f32cd0a68068a365
...
- ホーム画面のガジェット配置を詳細設定から設定可能です。配置情報で使われるガジェット名は[@tbl:homeprovider]で記載したプロバイダ名と一部内容が異なります。詳細設定内で、表示したい列をセミコロンで区切り、列内のガジェット名をカンマで区切ります。また設定項目のあるガジェットはコロンで区切ることで追加設定できます（省略した場合はデフォルトが適用される）。
	...
	一部のガジェットは内容の自動更新の設定が可能です。表～は自動更新間隔を設定する詳細設定項目です。

	No. | 設定項目名       | 設定値 | 内容 |
	---:|------------------|--------------|------|
	1   | XXXXXXXXXX  | ZZZZZZZZZZ  | CCCCCCCCCCCCCCC  |

	: ホーム画面の詳細設定の表 {#tbl:svhome}

	![](images/Tips.jpg){width=50px}　システム全体のデフォルト設定とユーザごとの個人設定が有り、ユーザごとの個人設定が設定されている場合は、個人設定が優先されます。
...
[@fig:fsample01] はプロフィールガジェット、[@tbl:tsample] は設定項目です。  

      	![竈門炭治郎](images/image_sample.png){#fig:fsample01 width=300px}
