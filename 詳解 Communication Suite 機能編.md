...
1. 基本タブの項目について
	...
	任意の通話属性が付与されている通話の場合、[1-2-2. 通話表示機能](#1-2-2. 通話表示機能) の [@tbl:callb2] の 『通話詳細』列の〇の内容が『名称』列を項目名としてデフォルトで表示されます。項目名については[@tbl:detaildc] の『通話情報に表示する追加の通話属性』から変更が可能です。『通話情報に表示する追加の通話属性』の設定書式は以下の通りです。
