...
通話終了後、[@fig:callp2] がポップアップ表示されて通話属性を登録することが可能です。通話リストからも登録できます。  
	...
	追加可能な通話属性は（[1-2-2. 通話[@tbl:callb]: OperatorAgent 表示する通話属性で利用可能な通話属性一覧](#1-2-2. 通話表示機能) 参照。）  
...
- ホーム画面のガジェット配置を設定できます。
	...
	![](images/Tips.jpg){width=50px}　『ホーム画面のレイアウト (配置情報)』はシステム全体のデフォルト設定とユーザごとの個人設定が有ります。ユーザがホーム画面の設定・レイアウト変更を行うまでは、デフォルト設定が適用されます。設定・レイアウト変更を行った際に、ユーザごとの個人設定が作成され、個人設定の内容が優先されるようになります。個人設定は、ユーザが設定・レイアウト変更を行った場合のみ更新されます。
...
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
		...
		---:|------|------------------|-  |   
		1  |ガジェット名   | Profile  | プロフィールガジェットを配置します。  |  
		2  | 設定なし  |   | このガジェットでは使用されません。【要確認】  |    
		3  |自動更新の有効化   | 0  |0:無効化,1:有効化   |   

		: プロフィールガジェットの書式説明 {#tbl:svhomeprofile}  

 	1. リンク  
ControlCenterで登録したリンクの設定です 。
...
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
		...
		No. | 設定項目      | デフォルトの設定 |設定内容|
		---:|------|------------------|-  |   
		1  |ガジェット名   | Announcement  | お知らせガジェットを配置します。  |  
		2  |表示件数の選択   |  5 | UI上は5,10,20件から選択、詳細設定では1~20件で設定可能です。  |    
		3  |既読のお知らせの表示   | 0  |0:表示しない,1:表示する   |   
  	:お知らせガジェットの書式説明 {#tbl:svhomeinfo}

 	1. 自分の通話  
ログインしているユーザの通話一覧です 。
ログインユーザの通話を、通話開始日時の新しい順で表示します。
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
		...
		No. | 設定項目      | デフォルトの設定 |設定内容|
		---:|------|------------------|-  |   
		1  |ガジェット名   | MyRecent | 自分の通話ガジェットを配置します。  |  
		2  |表示件数の選択   |  5 | UI上は5,10,20件から選択、詳細設定では1~20件で設定可能です。  |    
		3  |自動更新の有効化   | 0  | 0:無効化,1:有効化   |   
		4  |設定なし   |   | このガジェットでは使用されません。【要確認】  
		5  |設定なし   |   | このガジェットでは使用されません。【要確認】    
		6  |設定なし   |   | このガジェットでは使用されません。【要確認】
 	...
 	1. 自分の通話(IN)  
ログインしているユーザのINBOUND通話一覧です 。
ログインユーザのINBOUND通話を通話開始日時の新しい順で表示します。
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
		...
		No. | 設定項目      | デフォルトの設定 |設定内容|
		---:|------|------------------|-  |   
		1  |ガジェット名   | MyRecentInbound | 自分の通話(IN)ガジェットを配置します。  |  
		2  |表示件数の選択   |  5 | UI上は5,10,20件から選択、詳細設定では1~20件で設定可能です。  |    
		3  |自動更新の有効化   | 0  |0:無効化,1:有効化   |   
		4  |設定なし   |   | このガジェットでは使用されません。【要確認】  
		5  |設定なし   |   | このガジェットでは使用されません。【要確認】    
 	...
 	1. 自分の通話(OUT)  
ログインしているユーザのOUTBOUND通話一覧です 。
ログインユーザのOUTBOUND通話を通話開始日時の新しい順で表示します。
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
...
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
		...
		3   |表示件数の選択   |  5 | UI上は5,10,20件から選択、詳細設定では1~20件で設定可能です。  |    
		4  |自動更新の有効化   | 0  |0:無効化,1:有効化   |     
		5   |設定なし   |   |【要確認】    
		6   |設定なし   |   |【要確認】
...
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。
 	...
 	1. スキル  
ControlCenterで設定したスキルの設定です。  
ユーザ個人設定の『ホーム画面のレイアウト』での書式設定は以下です。

		```
		AgentSkill-"スキル番号":"自動更新の有効化"
		```

		No. | 設定項目      | デフォルトの設定 |設定内容
		---:|------|------------------|-  |   
		1  |ガジェット名   | AgentSkill  | スキルガジェットを配置する  |     
		2  |スキル番号   |   |ControlCenterで作成したスキルに割り当てられる番号です。1から作成した数だけ存在します。   |  
		3  |自動更新の有効化   | 0  |0:無効化,1:有効化   |   
...
[@tbl:svhomepara] の『感情の遷移モジュールに表示する感情スコア』『通話一覧モジュールに表示する感情スコア』 の設定書式は以下です。
	...
	感情スコア識別名については、[1-2-2. 通話表示機能　[@tbl:emoscore]: 保存する感情スコア](#1-2-2. 通話表示機能) を参照してください。
...
#### 2-3-1. 通話検索に利用する検索モジュールのプロバイダ  
 ...
 1.  検索モジュールのプロバイダ設定  
	検索モジュールに利用するプロバイダは、ControlCenter 詳細設定の [@tbl:provider0] の項目を編集して構成します。  
		...
		[@tbl:callb] の各種通話属性を、検索条件プロバイダとして検索モジュールに追加することができます。  
