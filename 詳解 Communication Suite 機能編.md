...
- 音声のマスクを有効にして、通話マスクを実行しても音声がマスクされない。
	...
	No. | 原因 | 対処 |
	----:|---------|-------|
	1 | 「再生に使用する音声ファイルのベースディレクトリのパス」のフォルダのACLに仮想ユーザ「SpeechVisualizer」 がアクセスできる権限がない。|該当フォルダのACLに対して「IIS AppPool\SpeechVisualizer」で追加可能な仮想ユーザ「SpeechVisualizer」を追加してください 。  
