...
#### 2-3-2. 通話検索の条件  
通話検索では通話の絞り込み条件や検索結果の並べ替え条件などを指定して検索します。  
プロバイダによって絞り込みで指定可能な条件が変わります。  

ここから修正中

 1. プロバイダごとに利用可能な検索条件  

	No. |プロバイダ名      | AND 検索| OR 検索|NOT 検索|備考|
	---:|-----------------|----------|----------|----------|----------|
	1   | プロジェクト|         |  
	2   | 検索対象の日付|       |
	3   | 検索対象の時間帯|        |  
...
「トマト -リンゴ」のように検索対象の単語と検索対象外の単語の組み合わせで利用可能です。  
日付や時間の検索条件については NOT 検索を利用することはできません。  

#### 2-3-3. 通話検索の結果

**1.通話に紐づく通話属性**  
検索結果で表示された通話に紐づいている通話属性が表示されます。([@fig:kensakukekka])  
表示する通話属性はControlCenterの詳細設定より追加することが可能です。([@tbl:tuuwazokusei0])  
...
通話属性を追加することで通話属性を追加前に保存された通話に対しても取得していた属性情報が表示されるようになります。    

**※通話属性は電話基盤、通話プロバイダ、コールシナリオなどの条件によって取得可否が変わります。  
どの通話属性が取得できるかは、導入環境で実際のコールシナリオを実施して確認することを推奨します。**  
