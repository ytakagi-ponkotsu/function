...
- 通話内容ビューの表示不具合のトラブルシューティング  
<br>
	**区分 A ： 通話フィルタで設定したキーワードが検出されたのにコマンドが実行されない**  
実行コマンドが Windows上で実行できないものが登録されているかの可能性があります。該当PCの「ファイル名を指定して実行」から登録した実行コマンドが実行できるか確認してください。

	**区分 B ： 〇〇〇の拡張子ファイルを呼び出すことができない**  
区分Aと同様に「ファイル名を指定して実行」から実行コマンドを確認してください。またブラウザに対象の拡張子が関連付けられているかを確認してください。
